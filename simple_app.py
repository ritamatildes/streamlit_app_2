...
def get_analysis_for_address(address):
    ...
    try:
        # 1. Get Population from Google Sheet
        ...
        # 2. Get CIRAC Risk from the Security Map API
        ...
        try:
            ...
            pass # If the structure isn't what we expect, out_cirac_cod remains None
        ...
        return f"Erro de rede ao obter dados (população, CIRAC ou POIs): {e}"
    except Exception as e:
        return f"Ocorreu um erro inesperado durante a recolha de dados: {e}"
    ...
    if final_class and out_pop and out_cirac_desc:
        return f"A morada que analisou ({address}) apresenta um potencial **{final_class}**: neste concelho ({out_municipality}) residem {out_pop} pessoas, o risco de inundação é **{out_cirac_desc}** (nível {out_cirac_cod}) e, num raio de 500m, existem {out_poi_count} pontos de interesse."
    else:
        return "Não foi possível concluir a análise. Um ou mais dados (população, risco CIRAC, ou POIs) não foram encontrados para este local."
...
with st.form("address_form"):
    ...
    submitted = st.form_submit_button("Analisar Morada")
